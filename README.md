...
- **可关闭自动打票**：首页和「采集时间与次数」新增同一个持久化开关。关闭后不再因缺票、到期、失败、空位或新请求自动采集，也不从普通回退回复静默收新票；已有主票、备用票、固定节点、等待和预算记录保留，手动打票仍可使用，无需重启。
- **固定节点不再自动切换**：固定节点即使连接失败也不标记为失败、不切到其它本地节点，后续仍尝试同一入口，方便在另一台代理设备中直接更换上游节点。手动停用仍会阻止使用；取消固定后恢复普通轮询和失败隔离。
...
连接重建只处理本地连接状态，不能保证故障节点或远端服务立即恢复。同一节点两次重建尝试至少间隔 10 秒，以避免重复构建；这不会阻塞聊天或给手动打票增加等待。账号拒绝、限流和手动停用仍按原规则处理；自动失败清单只用于非固定节点，固定节点始终保留同一入口。

## 二次开发中维护的功能

| 功能 | 行为 |
| --- | --- |
| 主票与备用票 | 按账号和模型隔离，默认绑定取得时的节点，可手动为主票更换当前出口；备用按取得顺序接替 |
| 手动丢弃 | 主票、备用票可单张丢弃，确认票号后执行；结果持久化，防止迟到响应把已丢弃票放回 |
| 手动打票 | 每次点击随机尝试一个可用节点，不受本地冷却、失败退避、备用错峰或小时预算的时间限制，不占用自动采集预算 |
| 自动采集 | 可在首页直接关闭；开启时按既定轮询、错峰与预算执行，关闭时保留已有票和固定节点，只响应手动打票 |
| 上游响应模型 | 显示响应声明的模型名，与请求模型不同时标记；未返回模型时明确提示 |
| 压缩请求带票 | 已有可用主票且开启注入时，V1/V2 压缩携带主票并使用其当前节点；不等待采集，不按普通生成规则拦截压缩响应 |
...
| 电脑 | r23 文件 | 启动入口 |
| --- | --- | --- |
| Windows Intel / AMD 64 位 | `ccodex-sleep-state-r23-windows-amd64.zip` | `start.cmd` |
| Windows ARM64 | `ccodex-sleep-state-r23-windows-arm64.zip` | `start.cmd` |
| Mac Apple Silicon | `ccodex-sleep-state-r23-darwin-arm64.tar.gz` | `start.command` |
| Mac Intel | `ccodex-sleep-state-r23-darwin-amd64.tar.gz` | `start.command` |
...
所有发布包只含程序、启动脚本、教程、版本信息和许可证，**不含维护者的节点、订阅、票、登录凭据、管理口令或运行日志**。下载后用 `ccodex-sleep-state-r23-SHA256SUMS.txt` 核对文件；源码包附锁定的 Go 依赖及其许可证。
...
- **网络错误**：检查节点和网络是否可用。固定节点会继续尝试同一入口，可直接在另一台代理设备中更换其上游节点；需要换本地节点时先取消固定或手动选择。非固定节点仍可手动恢复、切换或重新采集。
