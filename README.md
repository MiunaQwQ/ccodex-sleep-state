...
本分支正在验收：**拿到可用值后固定使用**、多订阅代理池、手动粘贴/TXT 导入、已用/失败清单、单节点重试，以及独立出口与大请求设置。首页给出下一步操作，复杂参数留在高级设置里。

现有版本不会自动升级；不改 `main`，不发布正式标签。试用前请看[升级与回退指南](docs/upgrade-0.4.0-preview.md)，运行边界见[合规与风险说明](docs/compliance.md)。
