...
func (e *Engine) probe(parent context.Context, h http.Header, route proxyroute.Route, models ...string) (turnstate.Token, int, time.Duration, error) {
	...
	client := &http.Client{Transport: route.ClientTransport(0, func(event proxyroute.ConnectionEvent) { e.logConnectionEvent(route.ID, event) }), CheckRedirect: func(*http.Request, []*http.Request) error { return http.ErrUseLastResponse }}
