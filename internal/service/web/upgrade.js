...
for(const id of ["reload-lifecycle-pool","load-egress"])$(id).addEventListener("click",()=>action(async()=>{await refresh();await loadLifecyclePool();}));
...
for(const [id,target] of [["pool-recycle","available"],["pool-disable","disabled"]])$(id).addEventListener("click",()=>action(async()=>{
 ...
 const r=await api("pool/change",{ids,state:target});$("pool-result").textContent=r.message;await loadLifecyclePool();
}));
