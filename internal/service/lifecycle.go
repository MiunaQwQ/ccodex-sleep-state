...
func (c *control) poolAPI(w http.ResponseWriter, r *http.Request, ctx context.Context) {
	...
	case "/admin/api/advanced":
		...
		if v.EgressMode == "fixed" {
			if v.EgressRoute == "" {
				fail(errors.New("固定出口需要选择一个节点"))
				return
			}
			// The old independent fixed-exit setting was easy to confuse with
			// state collection. A fixed node is now one single source of truth.
