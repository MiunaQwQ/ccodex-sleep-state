...
注入开启且已有可用主票时，V1/V2 压缩携带主票并保持来源出口；无票时仍不等待采集。新增 V1/V2、开关、出口绑定、网络失败后保留绑定与压缩不污染主备票的本地测试已通过。模型是否仍不一致，需要以更新后真实压缩的返回记录为准。

### 2026-09-19 r6 修复

此前版本把官方 `POST /responses/compact` 改写成普通 `POST /responses` 并追加 `compaction_trigger`；这会把独立压缩误送到普通生成容量路径。当前版本已移除该桥接：V1 原样转发，V2 仍按 `compaction_trigger` 识别；当时两者都跳过 state 注入，但保留当前 state 的出口绑定。此前“V1 404 / 桥接验收”的记录属于旧实现，不能代表 r6 的路径行为。新增原生路径、失败正文保持和压缩请求编码回归测试均已通过；真实上游仍需用 r6 独立验收。
